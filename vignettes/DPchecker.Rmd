...
DPchecker v0.3.1 runs two types of tests: metadata only tests and tests to determine whether the metadata and data files are congruent.  Metadata tests can be broken down in to two sub-categories, metadata compliance and metadata completeness. The tests run and the order in which they are run are listed below.
...
## Data Compliance
These functions check the data files for compliance. Please resolve any errors before uploading your data. 

* Data files do not contain any email addresses that constitute personally identifiable information (PII) ([`test_pii_data_emails()`](../reference/test_pii_data)
